...
library OracleUtils {
    ...
    function getLatestRoundDataStruct(AggregatorV3Interface _aggregatorV3)
        internal
        view
        returns (LatestRoundDataReturn memory _return)
    {
        (_return.roundId, _return.answer, _return.startedAt, _return.updatedAt, _return.answeredInRound) =
            _aggregatorV3.latestRoundData();
    }

    function setDecimals(Vm vm, AggregatorV3Interface _oracle, uint8 _decimals) public {
        vm.mockCall(
            address(_oracle), abi.encodeWithSelector(AggregatorV3Interface.decimals.selector), abi.encode(_decimals)
        );
    }
    ...
    function setPrice(Vm vm, AggregatorV3Interface _oracle, uint256 price_, uint256 _lastUpdatedAt)
        public
    ...
    function setPrice(Vm vm, AggregatorV3Interface _oracle, int256 price_, uint256 _lastUpdatedAt)
        public
        returns (int256 _price)
    {
        _price = int256(price_);
    ...
    function setPriceWithE18Param(Vm vm, AggregatorV3Interface _oracle, uint256 price_)
        public
        returns (int256 returnPrice)
    {
        (,,, uint256 _updatedAt,) = _oracle.latestRoundData();
